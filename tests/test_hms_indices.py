...
def test_failed_second_index_download_preserves_both_previous_files(tmp_path,monkeypatch):
    ...
    for name in ['kaupvisitala.csv','leiguvisitala.csv']:(tmp_path/name).write_text('previous')
    def get(url,**kwargs):
    ...
    for name in ['kaupvisitala.csv','leiguvisitala.csv']:assert (tmp_path/name).read_text()=='previous'
