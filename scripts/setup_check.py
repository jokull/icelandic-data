...
Verifies the Python version, that the Tier 0 dependencies import, which opt-in
tiers (maps, browser, pdf — see AGENTS.md "Requirement tiers") are present, that the
...
# Requirement tiers — keep in step with the table in AGENTS.md "Requirement tiers".
REQUIRED_MODULES = ["polars", "httpx", "duckdb", "openpyxl", "bs4", "typer", "iceaddr", "pdfplumber", "playwright"]
TIERS = {
    # name: (modules that must import, install command, what it unlocks)
    "maps": (["geopandas", "rasterio", "pyproj", "shapely", "matplotlib"], "uv sync --group maps", "map rendering, rasters"),
    "pdf": (["docling"], "uv sync --group pdf", "Docling AI table extraction"),
}
...
def check_tiers() -> None:
    """Report opt-in tiers 1 and 3. Tier 2 (browser) is reported by check_playwright."""
    for tier, (mods, install, unlocks) in TIERS.items():
        missing = []
        for name in mods:
            try:
                importlib.import_module(name)
                ...
                missing.append(name)
        if not missing:
            report(OK, f"tier {tier} installed ({unlocks})")
        else:
            report(WARN, f"tier {tier} not installed ({unlocks}) — `{install}`; missing {', '.join(missing)}")
...
def repair_skills_link() -> bool:
    ...
    if skills_link_resolves():
        ...
        if kind == "junction":
            report(WARN, "git status will now show .claude/skills as a typechange; that is expected — do not revert it")
        return True
...
def check_playwright() -> None:
    ...
    try:
        ...
        if path.exists():
            report(OK, "tier browser installed (Playwright Chromium)")
        else:
            report(WARN, "tier browser not installed (Power BI / Tableau scrapers) — `uv run playwright install chromium`")
    except Exception as e:  # noqa: BLE001
