...
CSV path (`?:format=csv` returns HTML). The [maskina](../maskina/SKILL.md) skill
...
3. **Data sourcing.** Most series are derived from Hagstofa PX-Web tables
   ([hagstofan](../hagstofan/SKILL.md) skill) plus additional Byggdastofnun surveys
...
- [hagstofan](../hagstofan/SKILL.md) — raw PX-Web for most underlying series
- [maskina](../maskina/SKILL.md) — documents the VizQL flow for extracting
  structured data from a Tableau Public workbook
- [reykjavik](../reykjavik/SKILL.md) — Reykjavík-municipality-specific data, complements
